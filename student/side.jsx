...
export default function ({ title, onUser }) {
    ...
    useEffect(() => {
        ...
        if (user_id === undefined || user_id === 'undefined' || user_id === null) {
            ...
            httpClient.get(Constants.REGISTER_USER + '/' + user_id)
                .then(response => {
                    if (response.status === 200) {
                        setUser(response.data.record);
                        onUser(response.data.record);
                    } else {
                        new Notification({
        ...
        <>
            ...
            <div className={"app__side"}>
                <div ref={loader_ref} className="side__wrapper">
                    <div className="title">{'Etudiant'}</div>
                    ...
                    {user === undefined ? <></> : (<>
                        ...
                        <div>
                            <span>{ 'Niveau : ' }</span>
                            <span className="text-bold">{ `${user.niveau}` }</span>
                        </div>
                        <div>
                            <span>{ 'Filière : ' }</span>
