 ...
 * @returns 
 */
 const RequestItemAssigned = ({ item, onValidate, onAssign, onReject }) => {
    ...
    const setMenu = () => {
        new FloatMenu(menu_ref.current, [
            { title: 'Traiter', onClick: () => onAssign(item), icon: 'DGfi-check' },
            { title: 'Rejeter', onClick: () => onReject(item), icon: 'DGfi-backspace' },
        ]);
    }
    return (
        <tr>
...
export default RequestItemAssigned;